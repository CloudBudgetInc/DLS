...
export default class AssessmentReportDli22 extends LightningElement {
    viewMode = true;
    @api picklistOptions;
    ...
    viewMode = true;
    showSpinner = false;    
    activeSections = ['SPEAKING PROFICIENCY', 'LISTENING COMPREHENSION', 'READING COMPREHENSION', 'OTHER','READING','LISTENING','SPEAKING','End-of-training goal (per TO)','Current Estimated ILR Rating','Progress Towards Goal ILR Rating', 'Strengths', 'Needs Improvement'];
    recordsToDelete = [];
    @track actionPlans = [];
    @track testScores = [];
    ...
    saveAssessmentReport(event){        
        ...
        if(isValidScores && isValidAssessmentReport){
            ...
            var saveChanges = {assessmentReport: this.assessmentReport, testScores: scoresToUpsert, recordsToDelete: this.recordsToDelete};
